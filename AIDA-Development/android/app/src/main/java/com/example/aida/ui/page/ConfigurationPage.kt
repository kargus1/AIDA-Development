...
import com.example.aida.ui.viewmodel.ConfigViewModel
...
fun ConfigurationPage(
    ...
    val isLidarConnected = uiState.connectionStates[ConnectionId.LIDAR] is ConnectionState.Connected
    val isSTTConnected = uiState.connectionStates[ConnectionId.STT] is ConnectionState.Connected
    val isCameraConnected = uiState.connectionStates[ConnectionId.VIDEO] is ConnectionState.Connected
    val isGestureConnected = uiState.connectionStates[ConnectionId.GESTURE] is ConnectionState.Connected
    val isJoystickConnected = uiState.connectionStates[ConnectionId.JOYSTICK] is ConnectionState.Connected

    Row(
        modifier = Modifier
            //.padding(top = barHeight) not needed?
            .fillMaxSize(),
    ) {
        ...
        Column(
            ...
            var ipInput by rememberSaveable { mutableStateOf(uiState.ip) }
            var portInput by rememberSaveable { mutableStateOf(uiState.port.toString()) }
            ...
            Row(
                ...
                LaunchedEffect(uiState.ip, uiState.port) {
                    ipInput = uiState.ip
            ...
            if (!uiState.isConnected) {
                // Button for connecting to the robot
                Button(
                    onClick = {
                        if (!isIpError || !isPortError) {
                            viewModel.connectToRobot(ipInput,portInput.toInt())
                        }
